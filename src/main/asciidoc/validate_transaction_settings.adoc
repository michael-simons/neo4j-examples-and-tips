...
The following configuration shown in <<SetupValidatingTransactionManager>> can be applied to both a purely Spring Data Neo4j configuration and in a Spring Boot based scenario.
The later provides you with the `SessionFactory` while you would have to take care of it withouth Spring Boot:

[source,java,indent=0]
[[SetupValidatingTransactionManager]]
...
Given a "broken" service like the one in <<BrokenService>>, the transaction manager refuses to execute the method.
