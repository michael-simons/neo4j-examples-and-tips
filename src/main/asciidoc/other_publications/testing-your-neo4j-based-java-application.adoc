...
NOTE: The full example howto use the test-harness for custom Neo4j extension is on Github: https://github.com/michael-simons/neo4j-sdn-ogm-tips/tree/master/examples/using-the-test-harness[using-the-test-harness].
...
NOTE: The full example howto use the Testcontainers Spring Data Neo4j based applications is on Github: https://github.com/michael-simons/neo4j-sdn-ogm-tips/tree/master/examples/using-testcontainers[using-testcontainers].
...
I recommend the following setup for an integration test with a Testcontainer:
...
<2> The extension will look for all attributes marked `@Container` and start and stop them according to their lifecycle
<3> Create (but don't start) a `Neo4jContainer`

Defining the Testcontainer as a `static` attribute defines a shared container.
This container is started before all tests and closed afterwards.
...
Also applicable for both plain OGM and SDN test is the way to provide a text-fixture.
...
----
<1> `neo4j` is the container we defined and started above. It provides access to the databases password
<2> The container provides an accessor to the Bolt url which contains a random port
...
The only thing you need to test Neo4j-OGM queries now is a Neo4j-OGM `SessionFactory`.
...
Again: No hardcoded password, no hardcoded Bolt-URL.
The Neo4j-Testcontainers provides this.
...
----
<1> This activates only Spring Datas repository layer and also provides Spring Boots JUnit 5 extensions
...
Given the same test data as in <<prepare-test-data>>, a not so boring test now is green:
