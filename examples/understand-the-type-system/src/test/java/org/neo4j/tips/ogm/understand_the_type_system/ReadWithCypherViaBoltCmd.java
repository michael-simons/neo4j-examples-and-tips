...
public class ReadWithCypherViaBoltCmd implements Function<Long, Long> {
	...
	public Long apply(final Long nodeId) {
		try (var session = driver.session()) {
			final Node node = session
					.readTransaction(tx -> tx.run(CYPHER_READ, Map.of("id", nodeId)).single().get("n").asNode());
			final Value testProperty = node.get(NAME_TEST_PROPERTY);
			LOGGER.info("Node with id {} was {}, property has value {} and is of type {}", node.id(),
					node.get(NAME_SOURCE_PROPERTY), testProperty, testProperty.getClass());
