...
public class TypeConversionTest {
    ...
    // tag::cypher-write[]
    static final String CYPHER_WRITE =
            "CREATE (n:TestNode) SET n.numericProperty = $numericProperty, n.source = $source RETURN n";
    // end::cypher-write[]
    // tag::cypher-read[]
    static final String CYPHER_READ =
            "MATCH (n:TestNode) WHERE id(n) = $id RETURN n";
    // end::cypher-read[]
    ...
    void initializeDatabase() throws IOException {
        ...
        this.graphDatabaseService = new GraphDatabaseFactory()
                .newEmbeddedDatabaseBuilder(neo4jDb)
                .setConfig(bolt.type, Connector.ConnectorType.BOLT.name())
                .setConfig(bolt.enabled, "true")
                .setConfig(bolt.listen_address, LOCAL_BOLT_URL)
                .newGraphDatabase();
    ...
    public void writingAndReadingViaDirectApi() {
        assertEquals(
                VALUE_TEST_PROPERTY,
                new WriteViaAPICmd(graphDatabaseService)
                        .andThen(new ReadViaAPICmd(graphDatabaseService))
                        .apply(VALUE_TEST_PROPERTY),
    ...
    public void writingAndReadingWithCypherOverEmbeddedConnection() {
        assertEquals(
                ...
                new WriteWithCypherViaEmbeddedCmd(graphDatabaseService)
                        .andThen(new ReadWithCypherViaEmbeddedCmd(graphDatabaseService))
                        .apply(VALUE_TEST_PROPERTY),
                "Returned not the same value as written"
        );
    }
    ...
    public void writingAndReadingWithCypherOverBoltConnection() {
        assertEquals(
                (Long) VALUE_TEST_PROPERTY.longValue(),
                new WriteWithCypherViaBoltCmd(driver)
                        .andThen(new ReadWithCypherViaBoltCmd(driver))
                        .apply(VALUE_TEST_PROPERTY),
                "Returned not the same value as written"
        );
    }
    ...
    class OGM {
        ...
        public void overEmbedded() {
            final SessionFactory sessionFactory =
                    new SessionFactory(new EmbeddedDriver(graphDatabaseService), this.getClass().getPackage().getName());
        ...
        public void overBolt() {
            var sessionFactory =
                    new SessionFactory(new BoltDriver(driver), this.getClass().getPackage().getName());
            final Session session = sessionFactory.openSession();
            assertAll(session.loadAll(TestNode.class).stream()
                    .map(node -> () -> assertEquals(VALUE_TEST_PROPERTY.longValue(), node.getNumericProperty()))
            );
